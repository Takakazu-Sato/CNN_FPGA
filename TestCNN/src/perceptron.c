...
/*HiddenLearn()関数　中間層の重み学習*/
void HiddenLearn(
	double *weight_hidden, //中間層の重み
	double *hidden_out, //出力層の出力
	int hidden_num //中間層の数
	double *input_data, //入力層のデータ
	int input_num,	//入力層の個数
...
)
{
	...
	for(i=0;i<hidden_num;++i){
		weight_out[i]+=ALPHA*hidden_out[i]*d;//重み学習
	}
	weight_out[i]+=ALPHA*(-1.0)*d;//閾値の学習
}
/*InitWeight_hidden()関数  中間層の重みの初期化*/
